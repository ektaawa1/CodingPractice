package patternBased.binarySearch.binarySearchOnAnswer;

/**
 * Input: candies = [5,8,6], k = 3
 * Output: 5
 * Explanation: We can divide candies[1] into 2 piles of size 5 and 3,
 * and candies[2] into 2 piles of size 5 and 1. We now have five piles of
 * candies of sizes 5, 5, 3, 5, and 1. We can allocate the 3 piles of size 5 to 3 children.
 * It can be proven that each child cannot receive more than 5 candies.
 *
 * Input: candies = [2,5], k = 11
 * Output: 0
 */
public class MaxCandiesAllocatedToKChildren {
    public int maximumCandies(int[] candies, long k) {
        //Each child gets the same number of candies
        //From one pile you can serve multiple children
        //You cannot combine piles for one child
        //You cannot split candies

        //LAST TRUE becoz we have to find Maximum x such that we can serve at least k children
        int low = 1;
        int high = 0;

        for (int candy : candies) {
            high = Math.max(high, candy);
        }

        while(low <= high){
            int mid = low + (high-low)/2;
            if(canAllocate(candies, k, mid)){
                low = mid +1;
            } else {
                high = mid -1;
            }
        }
        return high;
    }
    private boolean canAllocate(int[] candies, long k, int candiesNum){
        long served = 0;
        for(int candy : candies){
            served += candy/candiesNum;
        }
        return served >= k;
    }
}
//TC = O(n log M)

/**
 If each child gets x candies:
 From a pile of size p, we can serve:p / x   children

 Total children we can serve: sum(pile / x)

 If this ≥ k → x is feasible ✔
 */

/**
 * You are given a 0-indexed integer array candies. Each element in the array denotes a pile of candies of size candies[i].
 * You can divide each pile into any number of sub piles, but you cannot merge two piles together.
 * You are also given an integer k. You should allocate piles of candies to k children such that
 * each child gets the same number of candies. Each child can be allocated candies from only one pile of candies
 * and some piles of candies may go unused.
 * Return the maximum number of candies each child can get.
 */
